...
test("the browser app has no external module dependency before rendering", () => {
  ...
  assert.match(index, /<script src="app\.js\?v=20260724-label-fix" defer><\/script>/);
});
...
test("inspected space dropdown and export controls are present", () => {
  ...
  assert.match(app, /const roomNumberLabel = document\.querySelector/);
  assert.doesNotMatch(app, /roomNumber\.closest\("label"\)/);
  assert.match(app, /numberNotApplicable\.addEventListener/);
  assert.match(app, /function handleIssueSelectionChange/);
  assert.match(app, /function syncIssueCatalogSelections/);
  assert.match(app, /issueCatalogEl\.addEventListener\("change", handleIssueCatalogChange\)/);
  assert.match(app, /issueCatalogEl\.addEventListener\("click", handleIssueCatalogClick\)/);
