...
function buildingLabel(building) {
  const number = buildingNumbers[building];
  return number ? `${number}-${building}` : building;
}

function buildingFilenameLabel(building) {
...
function formatPartnerSummary(entry) {
  const issueSummaries = (entry.issues || [])
    ...
    .join("; ");
  return `${cleanTextCell(buildingLabel(entry.building))} ${cleanTextCell(entry.roomNumber)} -- ${issueSummaries}`;
}
